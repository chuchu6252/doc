...
*   当模拟进入夜间模式时， __路灯__ 会自动打开。灯光由地图开发人员放置，并可作为 [__carla.Light__](python_api.md#carla.Light) 对象访问。颜色和强度等属性可以随意更改。[__carla.LightState__](python_api.md#carla.LightState) 类型的变量 __light_state__ 允许在一次调用中设置所有这些。 

    路灯根据其属性 **light_group**（类型为 [__carla.LightState__](python_api.md#carla.LightState) ）的属性进行分类。可以通过一次调用检索 [__carla.LightManager__](python_api.md#carla.LightManager) 实例来管理灯光组。
...
!!! 笔记
    还可以使用天气部分中描述的 [environment.py](https://github.com/OpenHUTB/hutb/blob/hutb/PythonAPI/util/environment.py) 脚本实时设置灯光。
